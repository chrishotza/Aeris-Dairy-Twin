...
inp = data_dir / "full_alert_feed_synth_v2.csv"
out_events = data_dir / "managed_alert_events_synth_v2.csv"
out_incidents = data_dir / "incident_summary_synth_v2.csv"
out_summary = data_dir / "incident_management_summary_synth_v2.txt"
...
for (entity_level, entity_id), g in df.groupby(["entity_level", "entity_id"], sort=False):
    ...
    for _, row in g.iterrows():
        ...
        if active_incident_id is None:
            active_incident_id = f"S2INC_{incident_counter:05d}"
            incident_counter += 1
            active_opened_at = ts
...
lines = []
lines.append("INCIDENT MANAGEMENT SUMMARY SYNTH V2")
lines.append("===================================")
lines.append(f"raw_alerts: {len(df)}")
lines.append(f"managed_events: {len(events_df)}")
lines.append(f"incidents: {len(incidents_df)}")
lines.append("")
lines.append("event_types:")
for k, v in events_df["event_type"].value_counts().to_dict().items():
    lines.append(f"- {k}: {v}")
lines.append("")
lines.append("incident_max_severity:")
for k, v in incidents_df["max_severity"].value_counts().to_dict().items():
    lines.append(f"- {k}: {v}")

out_summary.write_text("\n".join(lines), encoding="utf-8")

print("\n=== INCIDENT MANAGEMENT SYNTH V2 ===")
...
print("\nincident preview:")
print(incidents_df.head(20).to_string(index=False))

print(f"\nSaved:")
print(f"- {out_events}")
print(f"- {out_incidents}")
print(f"- {out_summary}")
