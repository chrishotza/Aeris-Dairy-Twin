...
for (scenario, unit_id), g in unit_pred.groupby(["scenario","unit_id"]):
    dashboard_rows.append({
        ...
        "hours_green": int((g["pred_severity_reinforced"] == "GREEN").sum()),
        "hours_yellow": int((g["pred_severity_reinforced"] == "YELLOW").sum()),
        "hours_red": int((g["pred_severity_reinforced"] == "RED").sum()),
        "hours_concern": int(g["pred_severity_reinforced"].isin(["YELLOW","RED"]).sum()),
        "concern_rate": round(float(g["pred_severity_reinforced"].isin(["YELLOW","RED"]).mean()), 4),
        "red_rate": round(float((g["pred_severity_reinforced"] == "RED").mean()), 4),
...
leaderboard_out = OUT_DIR / "challenge_sim_reinforced_leaderboard_v2.csv"
group_out = OUT_DIR / "challenge_sim_reinforced_group_predictions_v2.csv"
unit_out = OUT_DIR / "challenge_sim_reinforced_unit_predictions_v2.csv"
scenario_out = OUT_DIR / "challenge_sim_reinforced_scenarios_v2.csv"
dashboard_out = OUT_DIR / "challenge_sim_reinforced_dashboard_v2.csv"
params_out = OUT_DIR / "challenge_sim_reinforced_best_params_v2.json"
animal_out = OUT_DIR / "animal_states_reinforced_v2.csv"
summary_out = OUT_DIR / "challenge_sim_reinforced_summary_v2.txt"
...
animal_reinforced.to_csv(animal_out, index=False)

with open(params_out, "w", encoding="utf-8") as f:
    json.dump(best_params, f, indent=2)

lines = []
lines.append("CHALLENGE SIM REINFORCED V2")
lines.append("===========================")
lines.append("")
